...
# Step 1: Order the premiums for every simulation
ordered_premiums <- premiums_info %>%
  arrange(Simulation, PV_Premium)

# Menyusun ulang nomor urut (jika diperlukan)
ordered_premiums$Order_PV_Premium <- seq_along(ordered_premiums$PV_Premium)

num_obs <- nrow(ordered_premiums)
index <- floor(0.95 * num_obs)

# Step 2: Calculate Value at Risk (VaR) and Tail Value at Risk (TVaR) for every simulation
var_tvar_results <- ordered_premiums %>%
  group_by(Simulation) %>%
  summarise(
    VaR = quantile(PV_Premium, 0.05),  # Reference the column without specifying the data frame
    TVaR = mean(PV_Premium[PV_Premium > VaR])  # TVaR is the average of values beyond VaR
  )

# Step 3: Calculate the average VaR and TVaR
average_var_tvar <- var_tvar_results %>%
  summarise(
    Average_VaR = mean(VaR, na.rm = TRUE),  # Adding na.rm = TRUE to handle potential NA values
    Average_TVaR = mean(TVaR, na.rm = TRUE)  # Adding na.rm = TRUE to handle potential NA values
  )
...
qqnorm(average_premiums$Average_PV_Premium, main = "Q-Q Plot: Perubahan Parameter Tau")
...
# Create a histogram
hist(average_premiums$Average_PV_Premium, probability=TRUE, col="lightblue", breaks=20, main="Histogram Perubahan Tau")
...
ggplot(average_premiums, aes(x = Simulation, y = Average_PV_Premium)) +
  ...
  labs(title = "Grafik Rata-Rata dengan Confidence Interval",
       x="Simulasi",
       y="Rata-Rata Premi (Perubahan Tau)") +
  theme_minimal()
